...
ProxyPool is written in **pure Go** with no cgo. That means any platform Go
...
```
bin/
├── proxypool-linux-amd64
├── proxypool-linux-arm64
├── proxypool-linux-arm       # ARMv6/v7 (Raspberry Pi, OpenWrt, etc.)
├── proxypool-darwin-amd64    # Intel Mac
├── proxypool-darwin-arm64    # Apple Silicon
├── proxypool-windows-amd64.exe
└── proxypool-windows-arm64.exe
```
...
| GOOS         | Path                                                |
|--------------|-----------------------------------------------------|
| `linux`      | `$XDG_CONFIG_HOME/proxypool/config.yaml` (default `~/.config/proxypool/`) |
| `darwin`     | `~/Library/Application Support/proxypool/config.yaml` |
| `windows`    | `%APPDATA%\proxypool\config.yaml`                   |
| `freebsd`    | `~/.config/proxypool/config.yaml`                   |
| `openbsd`    | `~/.config/proxypool/config.yaml`                   |
| `netbsd`     | `~/.config/proxypool/config.yaml`                   |
| everything else | `~/.config/proxypool/config.yaml`                |
...
1. `scp bin/proxypool-openwrt-mipsel root@router:/usr/bin/proxypool`
2. `chmod +x /usr/bin/proxypool`
3. Drop your config at `/etc/proxypool/config.yaml`
4. Start it: `/usr/bin/proxypool -config /etc/proxypool/config.yaml &`
...
- Use `unix` sockets instead of TCP for local-only deployments: pass
  `http_listen: "unix:/tmp/proxypool.sock"` (requires a 3-line patch to
